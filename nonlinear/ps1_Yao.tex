...
Using integration by parts
	...
	From the FOC, 
	\[P'(q) = V_{q}(q) = C_{q}(q,\theta) + \frac{F(\theta)}{f(\theta)}C_{q\theta}(q,\theta)\]
	Since $C_{q\theta}(q,\theta)$ is positive, the monopsony purchasing price is higher than the first best case where marginal price equals marginal cost. Because $V_{qq}(q) \leq 0$, the monopsony quantity $q(\theta)$ is lower than the first best quantity $q^{FB}$. Since $F(\underline{\theta}) = 0$, the lowest type is not distorted so $q(\underline{\theta}) = q^{FB}$.
